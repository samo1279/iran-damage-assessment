...
class OSINTEngine:
    ...
    def full_scan(self, since_date='2026-03-01'):
        """
        ...
        queries = OSINT_SEARCH_QUERIES
        for i, q in enumerate(queries):
            print(f"  [{i+1}/{len(queries)}] Searching: {q}")
            arts = self._gdelt_search(q, max_records=15)
            all_articles.extend(arts)
            if i < len(queries) - 1:
                import time
                time.sleep(6)  # Respect GDELT rate limits (1 req per 5s)
